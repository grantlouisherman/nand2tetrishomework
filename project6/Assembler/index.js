...
function Parser (instructions) {
    ...
    this.firstPassForSymbols = () => {
        ...
        while(instructionCounter < fileContents.length){
            ...
            if(isSymbol){
                ...
                .replace(")", "")
                .trim();
                let address = cleanedSymbol;
                // if symbol is not a direct address look it up
                if(!Number.isInteger(parseInt(address))){
                    const symbol = this.symbolMemmoryRegister(cleanedSymbol);
                    address = ramAddressTable[symbol];
                }
    ...
    this.handleCBinary = (CInstruction) => {
        ...
        return `${C_BINARY_HEAD}${comp()}${dest()}${jump()}`
    } 
